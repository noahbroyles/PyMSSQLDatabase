...
def getWSCustomers() -> list:
    wsCustomers = """begin
        SELECT WSCUSTOMER_ID, BILLCOMPANY
        FROM bestnest.WSCUSTOMER
        WHERE TPLCUSTOMER = 1
    end"""

    db = Database()
    results = db.query(wsCustomers)
    db.close()

    return [{'wsCustomerID': r.WSCUSTOMER_ID, 'companyName': r.BILLCOMPANY} for r in results]


def getWSCustomerIDs() -> list:
    wsCustomerIDs = """begin
        SELECT WSCUSTOMER_ID
        FROM bestnest.WSCUSTOMER
        WHERE TPLCUSTOMER = 1
    end"""

    db = Database()
    results = db.query(wsCustomerIDs)
    db.close()

    return [r.WSCUSTOMER_ID for r in results]


def checkCode(code: int, username: str, admin: bool = False) -> bool:
    db = Database()
    if admin:
        if db.query("SELECT ADMUSERCHKCODE AS ck FROM bestnest.ADMUSER WHERE ADMUSERNAME = ?", [username])[0].ck == code:
            db.close()
            return True
    else:  # If they ain't admin, they're wholesale
        if db.query("SELECT WSCUSTOMERCHKCODE AS ck FROM bestnest.WSCUSTOMER WHERE USERNAME = ?", [username])[0].ck == code:
            db.close()
            return True
    db.close()
    return False
